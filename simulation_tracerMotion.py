...
# initialize tracers 
NTracerSqrt = 10
NTracer = NTracerSqrt*NTracerSqrt

dxTracer = L/NTracerSqrt

xTracer = np.zeros((NTracer,1))
yTracer = np.zeros((NTracer,1))

# xTracer = np.random.rand(NTracer,1)*L
# yTracer = np.random.rand(NTracer,1)*L

# tracers arranged in a grid
for i in range(0,NTracer):
	xTracer[i,0] = 0.5*dxTracer + int(i/NTracerSqrt)*dxTracer
	yTracer[i,0] = 0.5*dxTracer + i%NTracerSqrt*dxTracer

vxTracer = np.zeros(NTracer)
vyTracer = np.zeros(NTracer)

xTraj = np.zeros((NTracer,1))
yTraj = np.zeros((NTracer,1))
...
for nStep in range(0,numSteps):
	...
	tauD = 0.5*tauDLow + 0.5*tauDHigh - (tauDHigh - tauDLow)*np.arctan(np.sin(2.0*np.pi*t*freq + np.pi/3.0)/steepnessTrans)/np.pi
	...
	k1Px, k1Py, k1ux, k1uy = rhs(dt,Px,Py,ux,uy,tauD)
	k2Px, k2Py, k2ux, k2uy = rhs(dt,Px + 0.5*k1Px,Py + 0.5*k1Py,ux + 0.5*k1ux,uy + 0.5*k1uy,tauD)
	k3Px, k3Py, k3ux, k3uy = rhs(dt,Px + 0.5*k2Px,Py + 0.5*k2Py,ux + 0.5*k2ux,uy + 0.5*k2uy,tauD)
	k4Px, k4Py, k4ux, k4uy = rhs(dt,Px + k3Px,Py + k3Py,ux + k3ux,uy + k3uy,tauD)
	...
	# determine velocity field for tracer advection
	vx, vy, vxF, vyF = calcVelo(Px,Py,ux,uy)
	
	# interpolate velocity field between grid points
	nVec = np.linspace(0,N*dx,num=N,endpoint=False)
	vxInterpolate = interpolate.RectBivariateSpline(nVec,nVec,vx)
	vyInterpolate = interpolate.RectBivariateSpline(nVec,nVec,vy)
	
	for i in range(0,NTracer):
		# determine interpolated velocity for every tracer accounting for periodic boundary conditions
		vxTracer[i] = vxInterpolate(xTracer[i]%L,yTracer[i]%L)
		vyTracer[i] = vyInterpolate(xTracer[i]%L,yTracer[i]%L)
		# move tracer according to local velocity
		xTracer[i] = xTracer[i] + dt*vxTracer[i]
		yTracer[i] = yTracer[i] + dt*vyTracer[i]
	
	# extend the trajectories
	xTraj = np.hstack([xTraj,xTracer])
	yTraj = np.hstack([yTraj,yTracer])
	
# save tracer trajectories
np.savetxt(folderName+'/tracerTrajectx%04d.dat'%(runNum),xTraj)
np.savetxt(folderName+'/tracerTrajecty%04d.dat'%(runNum),yTraj)
